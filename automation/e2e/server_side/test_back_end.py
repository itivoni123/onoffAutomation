...
class TestBackEnd(object):
    ...
    @pytest.fixture
    def rest_api_instance(self, url):
        return RestAPI(url)

    def test_create_task(self, rest_api_instance):

        payload = rest_api_instance.new_task_payload()
        create_task_id_res = rest_api_instance.create_task(payload)
        assert create_task_id_res.status_code == 200
        data = create_task_id_res.json()

        task_id = data["task"]["task_id"]
        get_task_id_res = rest_api_instance.get_task(task_id)
    ...
    def test_update_task(self, rest_api_instance):
        ...
        update_task_response = rest_api_instance.update_task(new_payload)
        assert update_task_response.status_code == 200
        # endregion Action\Update the task

        # region Get and validate the changes
        get_task_response = rest_api_instance.get_task(task_id)
        get_task_data = get_task_response.json()
        assert get_task_data["content"] == new_payload["content"]
        assert get_task_data["is_done"] == new_payload["is_done"]
        # region Get and validate the changes

    def test_delete_task(self, rest_api_instance):

        # region Create a task
        payload = rest_api_instance.new_task_payload()
        create_task_response = rest_api_instance.create_task(payload)
        assert create_task_response.status_code == 200
        task_id = create_task_response.json()["task"]["task_id"]
        # endregion Create a task

        # region Delete the task
        delete_task_response = rest_api_instance.delete_task(task_id)
    ...
    def test_list_tasks(self, rest_api_instance):
        ...
        user_id = payload["user_id"]
        list_task_response = rest_api_instance.list_task(user_id)
